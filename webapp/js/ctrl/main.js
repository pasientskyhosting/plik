...
plik.controller('MainCtrl', [
  ...
  function ($scope, $api, $config, $route, $location, $dialog) {
    ...
    $scope.load = function (id) {
      ...
      $api
        .getUpload($scope.upload.id, $scope.upload.uploadToken)
    ...
    $scope.uploadFiles = function () {
      ...
      _.each($scope.files, function (file) {
        ...
        $api
          .uploadFile($scope.upload, file, progress, $scope.basicAuth)
    ...
    var loadUser = function (promise) {
      promise
        ...
        .then(null, function (error) {
          if ($scope.config.noAnonymousUploads) {
